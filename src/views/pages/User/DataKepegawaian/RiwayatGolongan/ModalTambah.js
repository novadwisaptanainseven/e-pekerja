...
import { getSelectGolongan } from "src/context/actions/UserPage/SelectEditDataDiri";
...
import { insertRiwayatGolongan } from "src/context/actions/UserPage/RiwayatGolongan";

const MySwal = withReactContent(swal2);

const ModalTambah = ({ idPegawai, modal, setModal, dispatch }) => {
  const [loading, setLoading] = useState(false);
  const [pangkat, setPangkat] = useState([]);
  const { dashboardDispatch, dataDiriDispatch } = useContext(GlobalContext);
  ...
  const showAlertSuccess = () => {
    MySwal.fire({
      icon: "success",
      title: "Tambah Riwayat Golongan Berhasil",
  ...
  const showAlertError = (message) => {
    ...
    MySwal.fire({
      icon: "error",
      title: "Tambah Riwayat Golongan Gagal",
      text: err_message,
    }).then((result) => {
      setLoading(false);
    });
