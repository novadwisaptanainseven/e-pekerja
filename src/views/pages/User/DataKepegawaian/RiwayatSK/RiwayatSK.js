...
import { getRiwayatSK } from "src/context/actions/UserPage/RiwayatSK/getRiwayatSK";
import { GlobalContext } from "src/context/Provider";
import { deleteRiwayatSK } from "src/context/actions/UserPage/RiwayatSK";

const MySwal = withReactContent(swal2);

const RiwayatSK = ({ id, dataActive, dataDiriDispatch, pegawai }) => {
  ...
  useEffect(() => {
    if (!data) {
      if (dataActive === "riwayat-sk") {
        getRiwayatSK(user.id_pegawai, setLoading, setData);
      }
    }
  }, [id, data, dataActive, user]);
  ...
  const handleDelete = (id_riwayat_sk) => {
    MySwal.fire({
      ...
      if (res.isConfirmed) {
        deleteRiwayatSK(user.id_pegawai, id_riwayat_sk, setLoading, setData);
        MySwal.fire({
          icon: "success",
          title: "Terhapus",
      ...
      <div style={{ width: "100%" }} className="d-flex justify-content-between">
        ...
        <div>
          <CButton
            ...
            color="info"
            className="ml-2"
            onClick={() => printSK(user.id_pegawai)}
          >
          ...
          <CButton
            ...
            color="success"
            className="ml-2"
            onClick={() => exportExcel(`riwayat-sk/` + user.id_pegawai)}
          >
